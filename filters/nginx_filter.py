...
class NginxFilter(RegexFilter):
    ...
    def filter(self, message):
        self.regexps = [
            ...
            {"regex": """^(?:(?P<machine>.+): )?(?P<ip>.+) (?P<domain>.+)?\s*- - \[%s] "(?P<verb>.+?) (?P<path>.+?) (?P<http_version>.+?)" (?P<status>.+?) (?P<body_bytes_sent>.+?) "(?P<http_referer>.+?)" "(?P<user_agent>.+)(?: Completed in:(?P<nginx_response_time>.+))?""" % gp.HTTPDATE_},
        ]
